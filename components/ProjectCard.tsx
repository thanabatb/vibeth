  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Tap hint */}
        <span className="absolute bottom-2 right-2 text-[10px] text-white/30 opacity-0 group-hover:opacity-100 transition-opacity">
          tap to preview
        </span>
      </div>
      ...
      <div className="p-3">
        <div className="flex items-start justify-between gap-1 mb-1">
          ...
          {!isClaimed && !isUnavailable && (
            <span className="shrink-0 text-[10px] px-1.5 py-0.5 rounded bg-amber-400/10 text-amber-400 border border-amber-400/20 font-medium">
              unclaimed
            </span>
          )}
          {isClaimed && (
            <span className="shrink-0 text-[10px] px-1.5 py-0.5 rounded bg-blue-500/10 text-blue-400 border border-blue-500/20 font-medium">
              claimed
            </span>
          )}
          {isUnavailable && (
            <span className="shrink-0 text-[10px] px-1.5 py-0.5 rounded bg-red-500/10 text-red-400 border border-red-500/20 font-medium">
              unavailable
            </span>
          )}
        </div>
        ...
        <div className="mt-2 flex items-center justify-between">
          <span className="text-[10px] px-2 py-0.5 rounded-full bg-white/[0.05] text-gray-400 border border-white/[0.06]">
            {project.category}
          </span>
          {isClaimed && project.profiles && (
            <span className="text-[11px] text-blue-400 truncate max-w-[80px]">@{project.profiles.username}</span>
          )}
        </div>
      </div>
    </div>
...
function MobileRow({ project, onClick }: { project: ProjectWithProfile; onClick: () => void }) {
  ...
  return (
    <div
      onClick={onClick}
      className={`flex items-center gap-3 px-4 py-3 border-b border-white/[0.06] active:bg-white/[0.03] cursor-pointer ${isUnavailable ? 'opacity-50' : ''}`}
    >
      {/* Thumbnail */}
      <div
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          ...
          {!isClaimed && !isUnavailable && (
            <span className="shrink-0 text-[9px] px-1.5 py-0.5 rounded bg-amber-400/10 text-amber-400 border border-amber-400/20">
              unclaimed
            </span>
          )}
        </div>
        ...
        <span className="text-[10px] text-gray-600 mt-0.5">{project.category}</span>
      </div>
